...
/**
 * `game_over` for a versus player eliminated by mistakes while the race is
 * still on — fired at the moment they're out, not whenever the room
 * eventually finishes, since a winner emerging later would otherwise leave
 * their own loss untracked.
 */
export function versusEliminatedEvent(
  room: Doc<"rooms">,
  player: Doc<"players">,
  playerCount: number,
  mistakes: number,
  ...
  return {
    ...
    properties: {
      ...
      player_count: playerCount,
      duration_ms: room.startedAt
        ? clockElapsed({ ...room, startedAt: room.startedAt }, outAt)
        : 0,
      mistakes,
      hints: player.hints ?? 0,
    },
